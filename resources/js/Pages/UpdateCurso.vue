<template>
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-400 to-purple-600">
        <div class="w-full max-w-lg bg-white p-10 rounded-lg shadow-lg">
            <h1 class="text-4xl font-semibold text-gray-800 mb-8 text-center">Update Course</h1>
...
export default {
    props: {
        id: Number, // Receives the course ID from the backend via Inertia
    },
    setup(props) {
        ...
        const submit = () => {
            ...
            Inertia.post('/course/update-course', formData, {
                headers: {
                    'Content-Type': 'multipart/form-data' // Define que estamos enviando um arquivo.
                },
                onError: (error) => {
                    errors.value = error;
                },
                ...
                processData: false, // Não processar o FormData para JSON
                contentType: false,  // Não definir automaticamente o tipo de conteúdo
            });
        };
